...
async function createTags(userId, tagNames) {
    const tags = [];
...
async function connectDeckTags(deckId, tags) {
...
async function createDeckWithCards(userId, deckData) {
...
async function createStudyLogs(userId, createdDecks) {
    const allCards = createdDecks.flatMap((item) => item.cards.map((card) => ({
        ...card,
        deckId: item.deck.id,
    })));
    const logsToCreate = [
...
async function unlockDemoBadges(userId, badges) {
    const unlockedCodes = ['FIRST_STEP', 'DECK_FINISHER'];
    for (const badge of badges) {
        if (!unlockedCodes.includes(badge.code))
            continue;
